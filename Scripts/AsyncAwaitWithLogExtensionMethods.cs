...
namespace Kogane
{
	...
	/// </summary>
	public static class AsyncAwaitWithLogExtensionMethods
	{
		//================================================================================
		// デリゲート(static)
		//================================================================================
		...
		/// 処理の開始時と終了時にログを出力します
		/// </summary>
		public static async Task<T> WithLog<T>( this Task<T> task, string message )
		{
		...
		/// 処理の開始時と終了時にログを出力します
		/// </summary>
		public static async Task WithLog( this Task task, string message )
		{
			OnStartLog?.Invoke( message );
			await task;
			OnFinishLog?.Invoke( message );
		...
		/// </summary>
		public static async Task<T> WithTimeLog<T>( this Task<T> task, string message )
		{
		...
		/// 処理の開始時と終了時に経過時間付きのログを出力します
		/// </summary>
		public static async Task WithTimeLog( this Task task, string message )
		{
			OnStartTimeLog?.Invoke( message );
			var now = DateTime.Now;
			await task;
